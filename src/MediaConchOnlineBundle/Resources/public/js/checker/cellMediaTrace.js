        ...
        nodeMT.find('.mt-view').on('click', function(e) {
            ...
            if (!$('#modalTrace-' + fileId).length) {
                ...
                $('#modalTrace-' + fileId + ' .mt-dld').on('click', function(e) {
                    e.preventDefault();
                    window.location = checkerAjax.downloadReportUrl(fileId, 'mt');
                });
            }
        });
    ...
    var displayTree = function(fileId) {
        ...
        $('#trace-' + fileId).jstree({
            core: {
                ...
                data: {
                    url: checkerAjax.reportTreeUrl(fileId, 'mt'),
                    dataType: 'json',
                    error: function() {
                        displayReportError('#modalTrace-' + fileId);
                        $('#modalTrace-' + fileId + ' .mi-dld').prop('disabled', true);
                    }
                }
            },
