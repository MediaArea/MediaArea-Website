var checkerTable = (function() {
    ...
    var init = function() {
        result = $('#result-table').DataTable({
            ...
            columnDefs: [
                ...
                { orderable: false, searchable: false, width: '8%', targets: [3, 4] },
                { width: '8%', targets: [1, 5] },
                { width: '30%', targets: [0, 2] },
            ]
        });
    ...
    var addFile = function(fileName, fileId, formValues) {
        var node = result.row.add( [ '<span title="' + fileName + '">' + textUtils.truncate(fileName.split('/').pop(), 45) + '</span>',
            '', '', '', '',
