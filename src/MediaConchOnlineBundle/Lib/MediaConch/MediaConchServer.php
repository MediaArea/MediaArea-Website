...
class MediaConchServer
{
    ...
    public function __construct($address, $port, $apiVersion, LoggerInterface $logger, SettingsManager $userSettings)
    {
    ...
    protected function callApiHandler($uri, $method, $params, $responseString, $responseClass)
    {
        try {
            ...
            if (isset($response->$responseString)) {
                ...
                $responseClass = 'MediaConchOnlineBundle\Lib\MediaConch\\'.$responseClass;
    ...
    protected function callApi($uri, $method, $params)
    {
        $header = [];
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_URL, $this->getApiUrl($uri, $method, $params));
    ...
    protected function getApiUrl($uri, $method, $params)
    {
        $url = 'http://'.$this->address.'/'.$this->apiVersion.'/'.$uri;

        if ('GET' == $method && in_array($uri, ['checker_status', 'policy_get_policies']) && isset($params['id']) && is_array($params['id'])) {
